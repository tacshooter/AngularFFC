package cj.video {
	...
	public final class VideoFallback extends Sprite {
		...
		private function videoReady() {
			...
			TweenNano.to(master, 0.75, {alpha: 1, ease: Quint.easeOut});
		...
		private function showControls(event:MouseEvent):void {
			...
			if(controls.alpha != 1) TweenNano.to(controls, 0.75, {alpha: 1, ease: Quint.easeOut});
		...
		private function hideControls(event:Event):void {
			...
			if(!isFull) {
				...
				TweenNano.to(controls, 0.75, {alpha: 0, ease: Quint.easeOut});
		...
		private function removed(event:Event):void {
			...
			if(vHolder != null) {
				...
				TweenNano.killTweensOf(master);
				if(controls != null) TweenNano.killTweensOf(controls);
